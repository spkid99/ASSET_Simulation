...
def get_price(ticker):
    ...
    try: 
        t = yf.Ticker(ticker)
        # 💡 안정성을 위해 fast_info 대신 history 종가를 최우선으로 사용합니다!
...
for name, data in user_portfolio.items():
    ...
    if ticker:
        ...
        # 💡 [궁극의 방어막] 주가가 0원으로 오면 자산 폭락을 막기 위해 평균 매수 단가를 사용합니다!
        if raw_price > 0:
            ...
            backup_raw = st.session_state.price_backup.get(ticker, 0.0)
            if backup_raw > 0:
                current_price = backup_raw if is_korean else backup_raw * exchange_rate
            else:
                # 백업도 없으면? "내가 샀던 원금"을 그대로 현재가로 유지시켜 -100% 방어 (0% 유지)
...
if stock_details:
    ...
    if price_error_flag:
        st.info("💡 야후 파이낸스 해외 서버 지연으로 인해 일부 종목 주가를 과거 데이터(평균매수단가 등)에서 임시로 불러왔습니다. 자산은 안전합니다!")
else:
