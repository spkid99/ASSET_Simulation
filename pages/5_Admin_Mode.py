...
with tab_money:
    ...
    if not users_list: st.info("유저가 없습니다.")
        ...
        with st.container(border=True):
            target_user = st.selectbox("누구에게 지원금을 줄까요?", users_list, key="bonus_user")
            bonus_amt = st.number_input("지급할 지원금 (원)", min_value=0.0, step=1000.0, value=10000.0)
            if st.button(f"🚀 {target_user}에게 투자 지원금 {bonus_amt:,.0f}원 쏘기", use_container_width=True):
                ...
                st.success(f"{target_user} 지갑에 투자 지원금 {bonus_amt:,.0f}원 입금 완료!")
                st.rerun()

# 💡 [신규 탭] 특별 우대금리 지급 기능
with tab_interest:
    st.subheader("📈 특별 우대금리 지급 (티끌모아 태산 상 보상)")
    st.write("저축왕에게 약속한 특별 금리(%)를 입력하면, 현재 예금 잔액에 비례하여 이자가 계산되어 통장으로 바로 입금됩니다.")
    if not users_list: st.info("유저가 없습니다.")
    else:
        with st.container(border=True):
            target_user_int = st.selectbox("특별 이자 지급 대상 선택", users_list, key="int_user")
            
            # 대상자의 현재 예금 잔액 불러오기
            u_deposit = next((float(r.get('예금잔액', 0)) for r in balance_data if r.get('사용자') == target_user_int), 0.0)
            st.write(f"현재 **{target_user_int}**님의 은행 예금 잔액: **{u_deposit:,.0f} 원**")
            
            # 이율 입력 및 자동 계산
            int_rate = st.number_input("적용할 특별 우대 금리 (%)", min_value=0.0, step=0.1, value=5.0)
            calculated_interest = u_deposit * (int_rate / 100)
            
            st.info(f"🎁 **최종 지급될 이자 금액: {calculated_interest:,.0f} 원**")
            
            if st.button(f"💸 {target_user_int} 은행 통장에 이자 쏘기", use_container_width=True, type="primary"):
                if calculated_interest > 0:
                    supabase.table("balance").update({"예금잔액": u_deposit + calculated_interest}).eq("사용자", target_user_int).execute()
                    st.session_state.db_loaded = False
                    st.success(f"🎉 {target_user_int}님의 통장에 이자 {calculated_interest:,.0f}원이 성공적으로 꽂혔습니다!")
                    st.rerun()
                else:
                    st.warning("지급할 이자 금액이 없습니다. (현재 예금 잔액이 0원입니다)")
